...
the limitations of the 4 resolutions. For example, why is a high temporal resolution (frequent visits) often sacrificed for high spatial resolution (small pixels)


Methodology (Practical): How we load data into R (using terra for speed or stars for large datasets) and SNAP
. Mention the importance of resampling Sentinel data to match Landsat's grid for cross-comparison
.
Case Study Link: Referencing MacLachlan et al. (2017) regarding Urban Expansion in Perth
.
Critical Insight: Landsat is highly valued because it is the longest free temporal repository of consistent data, making it ideal for multi-decadal change detection
.
Policy Connection: How these spectral metrics (like NDVI or Tasseled Cap) support SDG Goal 11 by identifying urban green space or informal housing
.
...
One of the most captivating technical concepts was the Earth Observation (EO) Dashboard's dynamic scaling. The ability to seamlessly glide from a global overview to a granular streetscape is not merely a UI convenience but a sophisticated engineering feat known as the **Image Pyramid (or Tiled Web Map)**.   

...
* **F1-Score:** The harmonic mean of Precision and Recall. It is the gold standard for **Application-Ready Data** (like @brownetal Dynamic World).
...
![Training Process Visualization](D:/0.CASA_USS/Jiaying-2026 portfolio-book/images/train.png)
*Figure 1: The iterative process of defining training samples for urban classification. [@browntal]*


![Training Data and Spectral Signatures](D:/0.CASA_USS/Jiaying-2026 portfolio-book/images/tarning.png)
*Figure 3: Analyzing spectral signatures within training datasets to ensure class separation. [@browntal]*
...
**The Solution: Spatial Leave-One-Out (SLOO CV)**
Methods advocated by @wadoux2021 use distance-based buffers to ensure the test fold is geographically isolated from the training folds.
...
This brings us to a critical debate: **Often, classifiers on EO data can overcomplicate things.** * If there is a clear, spectral divide between bands (for example, a sharp NIR spike in healthy vegetation), **do we actually need a complex classifier?** * Can we just **threshold** the data (e.g., $NDVI > 0.4$)? 
...
![Interpretability-Accuracy Tradeoff](D:/0.CASA_USS/Jiaying-2026 portfolio-book/images/modeltradeoff.png)
...
To ensure we aren't just "mapping noise," I’ve integrated more rigorous validation standards into my workflow:
* **The Accuracy Standard:** Following *Barsi et al. (2018)*, I now prioritize **Precision (User's Accuracy)** and **Recall (Producer's Accuracy)** over simple Overall Accuracy.
* **The Kappa Debate:** Based on *Foody (2020)*, I’ve moved away from the Kappa coefficient, focusing instead on the **F1-Score** and **ROC Curves** to evaluate model performance, especially in unbalanced urban environments.
* **Spatial Cross-Validation:** Perhaps most importantly, I’ve learned that standard K-fold validation is "blind" to **Spatial Autocorrelation**. As *Lovelace et al. (2022)* and *Karasiak et al. (2022)* argue, we must use **Spatial CV (SLOO)** to prevent the model from "cheating" by using its proximity to training points to guess the answer.  
